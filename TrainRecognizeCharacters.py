...
def read_training_data(training_directory):
    ...
    for each_letter in letters:
        for each in range(10):#each letter 10 times
            ...
            flat_bin_image = binary_image.reshape(-1)#here 2D array changed to 1D array
            image_data.append(flat_bin_image)
            target_data.append(each_letter)

    return (np.array(image_data), np.array(target_data))

def cross_validation(model, num_of_fold, train_data, train_label):#accuaracy
...
cross_validation(svc_model, 4, image_data, target_data)#calls
